...
const Page: React.FC = () => {
    ...
    const [activeOpportunity, setActiveOpportunity] = useState<string | null>(null); // State for accordion
    ...
    useEffect(() => {
        if (opportunities) {
            const currentOpportunities = opportunities
                .filter((opportunity: IOpportunity) => new Date(opportunity.date) >= new Date())
    ...
    const currentOpportunities = allOpportunities.slice(indexOfFirstOpportunity, indexOfLastOpportunity);
                ...
                <div className="content-generic">
                    ...
                    {currentOpportunities.map((data) => (
                        <article key={data._id} className="border-b border-slate-200">
                            <button
                                onClick={() => setActiveOpportunity(activeOpportunity === data._id ? null : data._id)}
                            ...
                            >
                                ...
                                <span className="text-slate-800 transition-transform duration-300">
                                    <FontAwesomeIcon icon={activeOpportunity === data._id ? faMinus : faPlus} className="w-4 h-4" />
                                </span>
                            </button>
                            {activeOpportunity === data._id && (
                                <div id={data._id} className="max-h-96 overflow-hidden transition-all duration-500 ease-in-out">
                                    <div className="pb-5 text-sm text-slate-500">
                                        <p><b>Date:</b> {data.date}</p>
                                        <p><b>Bring:</b> {data.company}</p>
                    ...
                    <div className="flex justify-between mt-4">
                        <button onClick={handlePreviousPage} disabled={currentPage === 1} className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
                            Previous
                        </button>
